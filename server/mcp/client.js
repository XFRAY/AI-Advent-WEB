...
export class McpConnection {
  constructor({ serverPath = defaultServerPath, env = process.env, timeoutMs = 30_000 } = {}) {
    ...
    this.env = Object.fromEntries(['ALTEGIO_PARTNER_TOKEN', 'ALTEGIO_USER_TOKEN', 'ALTEGIO_LOCATION_ID', 'REPORTS_DIR'].filter(key => env[key]).map(key => [key, env[key]]));
    this.client = null;
    this.pending = null;
  }
  ...
  async open() {
    ...
    const client = new Client({ name: 'day-19-client', version: '1.0.0' });
